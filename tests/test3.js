...
for(let i = 0; i < repeatTimes; i++) {
    ...
    // // 查找轮廓
    // const maskPtr = __pin(__newArray(myModule.Int32Array_ID2, output))

    const contoursPtr = traceContours(outputPtr, newWidth, 0, newWidth - 1, 0, newHeight - 1, 250)
    const contourArray = __getArray(contoursPtr)
    // 将结果写入文件
    consoleDetail && fs.writeFileSync("./tests/newDose.json", JSON.stringify(contourArray))
}
